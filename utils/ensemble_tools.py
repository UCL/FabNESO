...
def createDictSweep(sweepPath, nDirs, destructive, copyDir, editFile, parameterDict):
    ...
    print(
        ...
        "Creating {2} configurations...".format(
            len(parameterDict.keys()), nDirs, nDirs ** len(parameterDict.keys())
        )
    )
    ...
    for parameter in parameterDict.keys():
        ...
        for i in range(nDirs):
            ...
            paramValue += (
                parameterDict[parameter][1] - parameterDict[parameter][0]
            ) / float(nDirs - 1)
    ...
    for comb in allConfigs:
        ...
        parameters = {}
        for parameter in parameterDict.keys():
            paramIndex = [item for item in comb if item.startswith(parameter)][0].split(
                "_"
            )[-1]
...
def encodeConditionsFile(inFileName, paramDict):
    ...
    for paramName in paramDict.keys():
        editParameter(inFileName, paramName, paramDict[paramName])
