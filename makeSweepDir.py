...
def createDictSweep(sweepPath,nDirs,destructive,copyDir,editFile,parameterDict ):
    print("{0} parameters found, requiring {1} divisions per parameter. Creating {2} configurations...".format(len(parameterDict.keys()),nDirs, nDirs**len(parameterDict.keys())))
    scanPoints = {}
    listAll = []
    for parameter in parameterDict.keys():
        ...
        for i in range(nDirs):
            ...
            paramValue += (parameterDict[parameter][1]-parameterDict[parameter][0])/float(nDirs-1)

    #Create the combinations of these parameters. It really feels like there should be a cleaner way of doing this, but I couldn't find it.
    combinations = list(itertools.combinations(listAll,len(parameterDict.keys())))
    allConfigs = []
    ...
    copyFiles = os.path.isdir(copyDir)
    #Now loop over each combination, create a directory, copy the configs and conditions, and edit the configuation to contain the correct parameters.
...
def encodeConditionsFile(inFileName,paramDict):
    for paramName in paramDict.keys(): editParameter(inFileName,paramName,paramDict[paramName])
...
def main():
    ...
    parser.add_argument("--parameterDict",help="An input dict of the parameters to be scanned.",default="")
    parser.add_argument("--dirPrefix",help="Prefix to call output dir",default="d")
    args = parser.parse_args()

    if args.parameterDict:
        #If this has been specified, we'll automatically create the directory tree as a multidimensional scan of these points
        parameterDict = eval(args.parameterDict)
        #Probably insert here a check that the dict is of the correct format before we start running the execution?
        createDictSweep(args.sweepPath,args.nDivs,args.destructive,args.copyDir,args.editFile,parameterDict)
